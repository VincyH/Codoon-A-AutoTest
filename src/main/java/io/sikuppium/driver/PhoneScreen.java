...
public class PhoneScreen implements Screen {
    ...
    public PhoneScreen(AppiumDriver driver) throws IOException {
        ...
        System.out.println("Dimension 1 :"+size);

    }

    public PhoneScreen(AppiumDriver driver,int x, int y, int width, int height) throws IOException {
        this.driver = driver;
        File screenshotFile = driver.getScreenshotAs(OutputType.FILE);

        BufferedImage full = ImageIO.read(screenshotFile);
        BufferedImage cropped = crop(full, x, y, width, height);
        size = new Dimension(1080,200);

        System.out.println("Dimension 2 :"+size);
    }
